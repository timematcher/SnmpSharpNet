...
namespace SnmpSharpNet
{
    ...
    public class IpAddress : OctetString, IComparable, ICloneable
    {
        ...
        public int GetClass()
        {
            ...
            if ((octet & 0x80) != 0 && (octet & 0x40) == 0)
                return ClassB; // Class B
            if ((octet & 0x80) != 0 && (octet & 0x40) != 0 && (octet & 0x20) == 0)
                return ClassC; // Class C
            if ((octet & 0x80) != 0 && (octet & 0x40) != 0 && (octet & 0x20) != 0 && (octet & 0x10) == 0)
                return ClassD;
            if ((octet & 0x80) != 0 && (octet & 0x40) != 0 && (octet & 0x20) != 0 && (octet & 0x10) != 0)
                return ClassE;
        ...
        {
            byte[] bitmask = {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01};
            var ip = _data;
