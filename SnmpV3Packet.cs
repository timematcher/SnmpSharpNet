...
namespace SnmpSharpNet
{
    ...
    public class SnmpV3Packet : SnmpPacket
    {
        ...
        /// </param>
        public void authNoPriv(byte[] userName, byte[] authenticationPassword,
            AuthenticationDigests authenticationProtocol)
        {
        ...
        public void authPriv(byte[] userName, byte[] authenticationPassword,
            AuthenticationDigests authenticationProtocol, byte[] privacyPassword, PrivacyProtocols privacyProtocol)
        {
            NoAuthNoPriv(userName); // reset authentication and privacy values and set user name
            _msgFlags.Authentication = true;
            _userSecurityModel.AuthenticationSecret.Set(authenticationPassword);
        ...
        public int decode(byte[] berBuffer, int length, byte[] authKey, byte[] privKey)
        {
            ...
            if (_msgFlags.Privacy && _userSecurityModel.EngineId.Length > 0)
            {
                ...
                var decryptedScopedPdu = privacyProtocol.Decrypt(encryptedScopedPdu, 0, encryptedScopedPdu.Length,
                    privKey, _userSecurityModel.EngineBoots, _userSecurityModel.EngineTime,
                    _userSecurityModel.PrivacyParameters);
                var tempOffset = 0;
        ...
        public byte[] GenerateAuthenticationKey()
        {
            ...
            if (_userSecurityModel.Authentication != AuthenticationDigests.None)
            {
                ...
                if (authProto != null)
                    return authProto.PasswordToKey(_userSecurityModel.AuthenticationSecret,
                        _userSecurityModel.EngineId);
            }
            return null;
        }
        ...
        public static SnmpV3Packet DiscoveryResponse(int messageId, int requestId, OctetString engineId,
            int engineBoots, int engineTime, int unknownEngineIdCount)
        {
        ...
        public static SnmpV3Packet BuildInformResponse(SnmpV3Packet informPacket)
        {
            if (informPacket.Version != SnmpVersion.Ver3)
                throw new SnmpInvalidVersionException(
                    "INFORM packet can only be parsed from an SNMP version 3 packet.");
