...
namespace SnmpSharpNet
{
    ...
    public class SimpleSnmp
    {
        ...
        public Dictionary<Oid, AsnType> Get(SnmpVersion version, Pdu pdu)
        {
            ...
            try
            {
                var param = new AgentParameters(version, new OctetString(_community));
        ...
        public Dictionary<Oid, AsnType> GetBulk(Pdu pdu)
        {
            ...
            try
            {
                var param = new AgentParameters(SnmpVersion.Ver2, new OctetString(_community));
                var result = _target.Request(pdu, param);
                if (result != null)
                    if (result.Pdu.ErrorStatus == 0)
                    {
        ...
        /// </example>
        /// <param name="version">
        ///     SNMP protocol version. Acceptable values are SnmpVersion.Ver1 and
        ///     SnmpVersion.Ver2
        /// </param>
        /// <param name="rootOid">
        ...
        public Dictionary<Oid, AsnType> Walk(SnmpVersion version, string rootOid)
        {
            ...
            while (lastOid != null && root.IsRootOf(lastOid))
            {
                ...
                if (version == SnmpVersion.Ver1)
                    ...
                    val = GetBulk(new[] {lastOid.ToString()});
