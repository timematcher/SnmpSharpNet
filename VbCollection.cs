...
namespace SnmpSharpNet
{
    ...
    public class VbCollection : AsnType, IEnumerable<Vb>
    {
        ...
        public VbCollection(IEnumerable<Vb> second)
        {
            ...
            //	_vbs.Add(v);
            //}
            Parallel.ForEach(second, v => { _vbs.Add(v); });
            _vbs = _vbs.OrderBy(v => v.Oid).ToList();
        }
        ...
        public Vb this[Oid oid]
        {
            get
            {
                if (!ContainsOid(oid))
                    return null;
                return _vbs.FirstOrDefault(v => v.Oid.Equals(oid));
            }
        }
        ...
        {
            get
            { return _vbs.FirstOrDefault(v => v.Oid.ToString().Equals(oid)); }
        }
        ...
        public void Add(IEnumerable<Vb> varList)
        {
            if (varList != null)
            {
                ...
                //	Add(v);
                //}
                //Code modified by Awais 
                Parallel.ForEach(varList, Add);
                //_vbs gets affected, needed to re-order it
                _vbs = _vbs.OrderBy(v => v.Oid).ToList();
            }
        }
        ...
        public bool ContainsOid(Oid oid)
        {
            ...
            //}
            //return false;
            //Code optimized by Awais 
            return _vbs.AsParallel().Any(v => v.Oid.Equals(oid));
        }
        ...
        public Oid[] OidArray()
        {
            ...
            //return o.ToArray();
            //Code optimized by Awais
            Parallel.ForEach(_vbs, v => { o.Add((Oid) v.Oid.Clone()); });
            o.Sort();
            return o.AsParallel().ToArray();
        }
